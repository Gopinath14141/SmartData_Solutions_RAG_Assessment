...
1. **One consistent retrieval failure.** "What was Apple's net income…" does not retrieve
   page 4, because that table's embedding is diluted across 23 line items — it ranks 26th
   dense, 23rd lexical, outside the candidate window. The system **refuses rather than
   guessing**, which is correct behaviour on a retrieval miss, but the answer is
   retrievable and should have been found. A cross-encoder reranker is the indicated fix
   (D7, unmeasured).

2. **The smaller model conflates same-labelled line items.** Asked for other
   income/(expense), net, it returns the `-13` component from Note 4 rather than the `-10`
   income-statement total — even when the question names the statement. A limitation of
   `gpt-4o-mini`, reported rather than excused.

3. **Tuning made things worse, twice.** Widening candidate windows to 40 and raising
   `top_k` to 12 were both measured and both degraded results; `top_k=12` raised the
   hallucination rate from 0% to 20%. Defaults kept. More context is not better.

4. **The figure corpus is one decorative logo.** The pathway is architecturally complete
   and exercised end to end, but thinly evidenced. No claim is made that data-figure
   interpretation has been demonstrated at scale.

5. **Three tables never resolve headers** (two are the table of contents, which has none)
   and one has corrupt cell geometry. All four are flagged, none silently indexed.

6. **Narrative answer quality is not automatically measured** (see above).

7. **Qdrant client/server version skew** produces a compatibility warning when the running
   server trails the pinned client. Harmless here; pin both for a clean log.

---

## Future improvements

- **Cross-encoder reranking**, targeted at limitation 1 — the diagnosis already exists.
- **Row-level table representations** alongside the whole-table chunk, so a single line
  item is retrievable without splitting the table.
- **A stronger text model** for questions that turn on distinguishing same-labelled items.
- **An LLM judge with a second provider** for narrative answers, with its limitations
  documented, to close the unmeasured gap.
- **Multi-document support** — the filing is configurable already, but the extraction
  heuristics are tuned to characteristics measured on this one.

---

## Assumptions

- The test link in the assignment refers to the AAPL 2022 Q3 10-Q from the
  `docugami/KG-RAG-datasets` repository, as specified in the project brief.
- "Figures" means images and data graphics in the document; this filing has only the
  former, and the distinction is handled explicitly rather than elided.
- Printed page numbers are the right citation unit, with PDF indices given alongside.
- The single document fits comfortably in a modern context window. Retrieval is justified
  here by precision, citation, and cost — not by corpus scale, and the documents say so.
